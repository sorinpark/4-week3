...
tree = soup.select_one('.ss_book_list>ul')
tree
...
"""
trees = soup.select(".ss_book_list>ul")
for tree in trees:
    try:
...
import pandas as pd
df = pd.DataFrame(datas,columns=['제목','링크','가격','평점'])
df
...
# csv 파일로 저장해 봅시다.
df.to_csv("aladin_bestseller.csv", index=False)
